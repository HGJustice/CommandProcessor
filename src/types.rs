...
impl Operations {
    pub fn increase(value: &mut u32, amount: u32) -> Result<u32, CommandErrors> {
        if amount == 0 {
    ...
    pub fn decrease(value: &mut u32, amount: u32) -> Result<u32, CommandErrors> {
        if amount == 0 {
    ...
    pub fn append(value: &mut String, input: &str) -> Result<String, CommandErrors> {
        if input.is_empty() {
    ...
    pub fn cut(value: &mut String, amount: usize) -> Result<(String, String), CommandErrors> {
        if amount == 0 {
...
impl Commands<u32> for CommandProcessor<u32> {
    ...
    fn undo(&mut self) -> Result<(), CommandErrors> {
        ...
        match last_operation {
            ...
            _ => Err(CommandErrors::InvalidOperationTypeOnData),
        }
    }
    fn redo(&mut self) -> Result<(), CommandErrors> {
        if self.current_position >= self.commands.len() {
    ...
    fn execute(&mut self, operation: Operations) -> Result<(), CommandErrors> {
        ...
        match operation {
            ...
            Operations::Truncate(amount, _) => {
                ...
                self.data = result;
                self.commands
                    .push(Operations::Truncate(amount, removed_text));
                self.current_position += 1;
                Ok(())
            }
